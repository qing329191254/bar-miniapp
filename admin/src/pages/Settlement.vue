...
function statusText(s: string) { return s === "GRANTED" ? "已发放" : s === "REVOKED" ? "已撤销" : s === "SKIPPED" ? "未发放" : s === "BLOCKED" ? "被拦截" : s; }
  ...
  <div class="settlement-page">
    ...
    <section class="card settlement-card">
      <div class="settle-head">
        <div class="settle-summary"><span class="pill executed">{{ data.executed ? executeDate : "待执行" }}</span><span class="summary-copy" v-if="blockedCount">快照已冻结 · {{ blockedCount }} 张奖励被整批拦截，一张未发</span><span class="summary-copy" v-else-if="data.executed">快照已冻结 · 发放 {{ data.summary?.granted || 0 }} 张宝箱卡（战队 {{ data.summary?.team || 0 }} + 个人 {{ data.summary?.personal || 0 }}）</span><span class="summary-copy" v-else>尚未生成结算快照，请先查看下方预览</span></div>
      ...
      <div class="table-wrap"><table class="tb2 settlement-table"><thead><tr><th>获奖对象</th><th>类型</th><th>奖励</th><th>碎片校验</th><th>状态</th><th class="col-op">操作</th></tr></thead><tbody>
        <tr v-for="r in data.rows" :key="r.id" :class="{blocked:r.status==='BLOCKED'}"><td><b>{{ r.target }}</b><div class="tiny">{{ r.nick }}</div></td><td><span class="pill type-pill" :class="typeClass(r)">{{ labels[r.type] || r.type }}</span></td><td>{{ r.desc }}<div v-if="r.forceReason" class="tiny force-reason">强制发放：{{ r.forceReason }}</div></td><td class="shard">{{ Number(r.sh || 0).toLocaleString("en-US") }}</td><td><span class="pill status-pill" :class="`status-${String(r.status).toLowerCase()}`">{{ statusText(r.status) }}</span></td><td class="col-op"><button v-if="boss && ['GRANTED','BLOCKED'].includes(r.status)" class="btn ghost revoke-btn" @click="openRevoke(r)">撤销</button><span v-else class="tiny">—</span></td></tr>
    ...
    <section class="card preview-card"><div class="st">按当前规则预览发放 <button class="preview-rule-link" @click="router.push('/settlecfg')">规则可在「榜单与奖励规则」配置</button></div><div class="preview-meta"><span>本次将发放 <b :class="{red:preview.blocked}">{{ preview.count || 0 }}</b> 张</span><span>单次自动结算上限 {{ preview.cap || data.cfg?.settleCap || 20 }} 张</span><span class="pill" :class="preview.blocked ? 'preview-blocked' : 'preview-ok'">{{ preview.blocked ? "超过上限，执行后将整批拦截" : "当前未超限" }}</span></div><div class="preview-detail"><p>统计维度：{{ preview.cfg?.rankDim==='MONTH'?'月（按自然月累计）':'周（周一 00:00 重置）' }} · 个人奖励：前 {{ preview.cfg?.rankRange||data.cfg?.rankRange||3 }} 名 · 战队奖励：{{ preview.cfg?.teamReward?'开':'关' }} · 叠加：{{ preview.cfg?.stack?'可':'否' }}</p><p>模拟名单（碎片{{ preview.cfg?.rankDim==='MONTH'?'月':'周' }}榜）：{{ personalPreviewText }}</p><p>战队奖励：{{ teamPreviewText }}</p><p v-if="preview.missingRanks?.length" class="preview-missing">第 {{ preview.missingRanks.join('、') }} 名未配置奖励卡型，结算时将跳过。</p></div><div class="preview-help">预览仅用于核对本次结算人数与奖励内容，不会生成或发放卡券。</div></section>
