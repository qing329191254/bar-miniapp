...
const busy = ref(false), snapshot = ref<any>(null);
...
const memberOpts = computed(() => [
  { value: "", label: "请选择会员" },
  ...members.value.map((m: any) => ({ value: m.id, label: `${m.nick} · ${m.no}` })),
]);
const templateOpts = computed(() => [
  { value: "", label: "请选择奖励" },
  ...templates.value.map((t: any) => ({ value: t.id, label: t.name })),
]);
...
async function forceGrant() {
  ...
  catch (e: any) { showToast(e.message || "强制发放失败", true); } finally { busy.value = false; }
}
...
async function grantManual() {
  ...
  busy.value = true;
  try { await api("/admin/settlement/manual", { method: "POST", body: { data: { ...manual.value, reason: manual.value.reason.trim() } } }); manualOpen.value = false; manual.value = { uid: "", tplId: "", reason: "" }; showToast("手动补发成功，奖励已进入会员卡包"); await refreshAll(); }
  catch (e: any) { showToast(e.message, true); } finally { busy.value = false; }
}
...
<template>
  ...
  <div class="settlement-page">
    ...
    <div class="note settlement-rule-note">
      <div class="note-body">
        <p><b>名次占位制：</b>同一名次并列时，每位获奖者都会获得该名次对应的奖励；后续名次按实际占位顺延，超出配置范围不发奖。极端并列人数不设上限，实际发放总量以下方预览为准。<b>发奖以冻结快照为准</b>，结算后调整战队不会影响已发放奖励。</p>
        <p><b>发放张数上限：</b>单次自动结算发放卡券总数超过 <b>{{ preview.cap||data.cfg?.settleCap||20 }} 张</b>时，整批标记为「被拦截」、一张不发。它与「名次范围」是两个独立约束：并列和战队奖叠加可能使实际张数远高于名次范围。</p>
      </div>
    </div>
