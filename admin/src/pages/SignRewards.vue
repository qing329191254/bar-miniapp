...
async function remove(rule: any) { if (!window.confirm(`确认删除连续 ${rule.days} 天奖励？`)) return; try { await api(`/admin/sign-rules/${rule.id}`, { method: "DELETE" }); if (selectedId.value === rule.id) closeEdit(); await load(); } catch (e: any) { showToast(e.message || "删除失败", true); } }
onMounted(load);
</script>
