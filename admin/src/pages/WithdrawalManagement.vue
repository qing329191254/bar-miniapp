...
<template>
  <div>
    ...
    <div class="note rd">
      <b>本页仅供查询与服务分析，不支持确认发放。</b>提分兑付请由店员在<b>商家移动端「待办」页当面完成</b>：核对顾客提供的单号后四位，交付物品后再确认。这样可确保积分扣减与实际交付同步完成，避免产生争议。
    </div>
    ...
    <AppAsyncPage
      ...
      :data="data"
      :err="err"
      :skeleton="{ showHeader: false, showExtraCard: true, tableCols: 8, tableRows: 8 }"
      @retry="load()"
    >
      ...
      <div class="card timeout-card" :style="timeoutCardStyle">
        ...
        <div class="tiny timeout-note">
          <b>超时关闭率可用于观察吧台响应效率。</b>当该指标持续升高时，建议检查员工排班与待办处理流程。单据超时后，冻结积分会全额退回会员账户，不会产生积分损失。
        </div>
      </div>
      ...
      <div class="note">
        <b>状态说明：</b>提分单会进入待确认、已发放、已驳回、已取消或超时关闭。除「已发放」外，其他结果都会将冻结积分全额退回会员账户。<br />
        <b>频繁超时限制：</b>同一会员在 24 小时内超时关闭超过 3 次，将暂时无法继续提交；限制会随时间自动解除，无需人工操作。<br />
        <b>等待时长：</b>已完成单据按提交到发放或关闭的时间计算；待确认单据按提交至当前的时间实时更新。
      </div>
    </AppAsyncPage>
  </div>
</template>
